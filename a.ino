...
void loop() {
  ...
  Serial.println("lightIntensity: " + String(lightIntensity) + " lux");
  ...
  if (mySensor.update() != 0) {
    ...
    airTemp = mySensor.temperatureC;
    airHumid = mySensor.humidity;
  }
...
float lightIntensity, bool status, String date) {
  ...
  {
    // POST 요청 설정
  http.begin("http://192.168.1.134/api/post-sensor-data");
  http.addHeader("Content-Type", "application/json");
